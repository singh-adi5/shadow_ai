"""
Shadow AI Detector — Production Pipeline Orchestrator v3
=========================================================
...
Architecture (addressing all three production gaps from review):
  ...
  Gap 1 — O(N) Heap Allocation → FIXED
  ...
  Gap 2 — GIL-Bound Sequential NLP → FIXED
  ...
  Gap 3 — Shared Mutable State / Race Conditions → FIXED
    ...
    Policy engine remains stateless (unchanged from v2).
...
class ShadowAIDetectorPipeline:
    ...
    def run(self, num_logs: int = 1_000, source_file: Optional[Path] = None) -> int:
        ...
        print("\n" + "=" * 72)
        print("🔒  SHADOW AI DETECTOR — PRODUCTION PIPELINE v3")
        print("=" * 72)
        ...
        try:
            if source_file is not None:
                if not source_file.exists():
                    # Previously fell through to synthetic generation on a
                    # typo'd/missing --source path, silently scanning seeded
                    # fake PII and reporting it as real threats with exit 0.
