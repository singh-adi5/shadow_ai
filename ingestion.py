...
Addresses Gap 1 from architecture review:
...
from config import COMPILED_AI_PATTERNS, PII_QUICK_PATTERN, config
...
#
# Both the AI-domain check and the PII check reuse config.py's single source
# of truth (COMPILED_AI_PATTERNS / PII_QUICK_PATTERN) — this module used to
# keep its own separate, unbounded copies (in particular an EMAIL_ADDRESS
# alternative with overlapping quantified character classes that backtracked
# catastrophically: 9.4s on a 40KB adversarial payload). See config.py for
# the bounded pattern definitions.
# ---------------------------------------------------------------------------
...
def _passes_prefilter(raw_line: str) -> bool:
    ...
    """
    return bool(
        any(p.search(raw_line) for p in COMPILED_AI_PATTERNS)
        or PII_QUICK_PATTERN.search(raw_line)
    )
