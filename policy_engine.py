...
def _utc_now_iso() -> str:
    """UTC timestamp matching the 'Z'-suffixed format used everywhere else
    in the pipeline (scan results, audit log). Previously this module used
    naive datetime.now() (host-local time) for alert_id/timestamp while the
    rest of the system stamped UTC — skewing SIEM correlation by the host's
    UTC offset and making alert_id collision-prone across timezones."""
...
def rule_department_restriction(
    ...
    if department in restricted_depts and engine.is_ai_endpoint(destination) and entities:
        return PolicyAlert(
            alert_id       = f"POLICY-DEPT-{_utc_now_millis()}",
            timestamp      = _utc_now_iso(),
            log_id         = log_id,
            user_id        = log_entry.get("user_id", "UNKNOWN"),
...
def rule_after_hours_access(
    ...
    if (is_weekend or is_after_hours) and engine.is_ai_endpoint(destination):
        ...
        return PolicyAlert(
            alert_id       = f"POLICY-HOURS-{int(now.timestamp() * 1000)}",
...
def rule_high_volume_exfiltration(
    ...
    if (
        ...
        return PolicyAlert(
            alert_id       = f"POLICY-VOL-{_utc_now_millis()}",
            timestamp      = _utc_now_iso(),
            log_id         = log_id,
            user_id        = log_entry.get("user_id", "UNKNOWN"),
            department     = log_entry.get("department", "UNKNOWN"),
