...
Design principles:
  - Pydantic v2 models for all external I/O (FastAPI, file ingestion)
...
# Bare hostname (api.openai.com) OR a fully-schemed URL (https://api.openai.com/v1/...).
# Real proxy logs commonly capture the full URL, not just the host — the
# previous pattern had no `https?://` branch, so every schemed destination_url
# was rejected at the ingestion boundary.
_DESTINATION_URL_RE = re.compile(
    r"^(?:https?://)?[a-zA-Z0-9.\-]+(?::[0-9]{1,5})?(?:/.*)?$", re.IGNORECASE
)
...
class ProxyLog(BaseModel):
    ...
    def validate_url(cls, v: str) -> str:
        ...
        # Accept bare hostname (e.g. api.openai.com) or a full https?:// URL
        if not _DESTINATION_URL_RE.match(v):
